...
A day the normal routine does NOT happen — "no school", "no classes", "campus closed", "office closed", a holiday, a professional-development / in-service day, a snow day, an early-release day — is an ADVISORY, never an event and never a task. Nobody attends a closure; it exists only to remind the family the usual pattern is off. It has NO time-of-day, NO arrival_time, and person_hint = null (a household note), even when the email comes from one child's school. US schools and offices are generally closed on the US federal holidays: New Year's Day (Jan 1), Martin Luther King Jr. Day (3rd Mon Jan), Presidents' Day (3rd Mon Feb), Memorial Day (last Mon May), Juneteenth (Jun 19), Independence Day (Jul 4), Labor Day (1st Mon Sep), Columbus / Indigenous Peoples' Day (2nd Mon Oct), Veterans Day (Nov 11), Thanksgiving (4th Thu Nov), Christmas (Dec 25). Do NOT emit a separate event for a holiday that merely appears in a school-calendar list; a single "no school — <holiday>" advisory is enough.
...
Set "time" whenever the source gives or clearly implies a start time — the entry is otherwise treated as all-day. Only leave time null for genuinely all-day things: holidays, first/last day of school, spirit/dress-up days, multi-day spans. If an email states a time anywhere for an event, that event gets that time. If the only clock time given for an event is a "report by" / "arrive by" time (and maybe an end time) with no separate start — e.g. "players arrive by 9am, done by noon" — use that report time as the start "time" so the event is timed, not all-day, and leave arrival_time null (a distinct arrival_time only makes sense when it is earlier than the start).
